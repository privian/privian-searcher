...
export class Searcher {
	...
	readonly md: MarkdownIt = new MarkdownIt({
	...
	async getTOC(): Promise<ITOCItem[]> {
		...
		const sections = await this.selectAll<{
			...
			level: number | null;
			title: string;
		}>(`SELECT id, doc, level, title FROM sections WHERE title IS NOT NULL`);
		return docs.map((doc) => {
			return {
				id: doc.id,
				title: doc.title,
	...
	async getDoc(idOrUrl: number | string): Promise<IDoc> {
		...
		const doc = await this.selectOne<IDbDoc & {
			...
			imageDocType: string | null;
			imageDocUrl: string | null;
		}>(`SELECT
				docs.*,
		...
		if (!contents) {
			const sections = await this.selectAll<{
				...
				level: number | null
				title: string | null;
			}>(`SELECT * FROM sections WHERE doc = ? ORDER BY id ASC`, [doc.id]);
	...
	async listDocs(userOptions?: Partial<IListDocsOptions>): Promise<{ docs: Partial<IDoc>[], entities: IEntity[] | null }> {
		...
		let entities: IEntity[] | null = null;
	...
	async search(term: string, userOptions?: Partial<ISearchOptions>): Promise<ISearchItem[]> {
		const options: ISearchOptions = Object.assign({
			limit: 100,
			snippetSize: 64,
		}, userOptions);
		const sql = `SELECT
		...
		const items = await this.selectAll<IDbSection & {
			...
			docPublishedAt: Date | null;
			docTitle: string | null;
			docType: string | null;
			docUrl: string;
			docSummary: string | null;
			imageDocCrawl: boolean;
			imageDocCrc: string;
			imageDocSize: number | null;
			imageDocType: string | null;
			imageDocUrl: string;
			imageSectionCrawl: boolean;
			imageSectionUrl: string;
			linkSectionCrawl: boolean;
			imageSectionCrc: string;
			linkSectionCrc: string;
			linkSectionUrl: string;
			score: number;
			snippet: string;
		}>(sql, [term]);
	...
	renderMarkdown(contents: string, removeImages: boolean = false, removeAnchorLinks: boolean = false): string {
		const ellipsis = '...';
	...
	async replaceLinks(contents: string): Promise<string> {
		...
		const docs = await this.selectAll<{
			...
			id: number;
			url: string;
		}>(`SELECT id, crawl, crc, url FROM docs WHERE id IN (${ids.map(() => '?')})`, [...ids]);
		let cursor = 0;
		let result = '';
		for (let item of match) {
			const index = item.index || 0;
			const link: ILink | undefined = docs.find((d) => d.id === +item[1]);
